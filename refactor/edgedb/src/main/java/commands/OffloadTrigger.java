...
public class OffloadTrigger implements ICommand {
    ...
    public void Handle(IRequest req, IResponse res) {
        ...
        if (usedMemory >= offloadTopThreshold) {
            Logger.info("Setting status of node to reject offloads");
            ConfigurationDAO.rejectOffloading();

            Logger.info("Triggered offload");
            returnValue = manageOffload();
        } else if (usedMemory <= onloadThreshold) {
            Logger.info("Setting status of node to accept offloads");
            ConfigurationDAO.acceptOffloading();

            Logger.info("Triggered onload");
            manageOnload();
            returnValue = -1;
        } else {
            Logger.info("Node is healthy, nothing is triggered");
        }
